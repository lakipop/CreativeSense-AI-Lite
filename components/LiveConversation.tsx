...
const LiveConversation: React.FC<LiveConversationProps> = ({
  ...
  const [realtimeTranscript, setRealtimeTranscript] = useState<{user: string, model: string}>({ user: '', model: '' });
  ...
  const [descriptionStyle, setDescriptionStyle] = useState<DescriptionStyle>(initialStyle);
  ...
  const handleNewSession = () => {
    ...
    localStorage.setItem(CONVERSATION_HISTORY_KEY, JSON.stringify(updatedSessions));
  };
  ...
  useEffect(() => {
    if (activeSessionId && transcriptionHistory.length > 0) {
      setSessions(prev => {
        const updated = prev.map(s => s.id === activeSessionId ? { ...s, transcript: transcriptionHistory } : s);
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const systemInstruction = (styleConfig.systemInstruction('') || '') + (isSinhala ? ' Respond in Sinhala. If input is not Sinhala, translate the response into Sinhala.' : '');
            ...
            if (message.serverContent?.turnComplete) {
                setRealtimeTranscript(currentTranscript => {
                    ...
                    if (hasAudio && !finalTurn.model.trim()) {
                        finalTurn.model = '[Audio response could not be transcribed due to safety settings]';
                    }
  ...
  return (
    <div className="flex h-full w-full bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans overflow-hidden">
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
          <h1 className="font-bold text-lg">Gemini Live</h1>
          <button className="md:hidden p-1" onClick={() => setIsSidebarOpen(false)}>✕</button>
        </div>
        
        <div className="p-3">
          <button 
            onClick={handleNewSession}
            className="w-full py-2 px-4 bg-primary-600 hover:bg-primary-500 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
          >
            <span>+</span> New Chat
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {sessions.map(session => (
            <div 
              ...
              className={`p-3 rounded-lg cursor-pointer border transition-all ${activeSessionId === session.id ? 'bg-primary-50 dark:bg-zinc-800 border-primary-200 dark:border-zinc-700' : 'bg-transparent border-transparent hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
            >
              <div className="flex justify-between items-start">
                 <span className="text-sm font-medium truncate w-full">{session.title}</span>
                 <button onClick={(e) => { e.stopPropagation(); handleDeleteSession(session.id); }} className="text-zinc-400 hover:text-red-500">
                   ×
                 </button>
              </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        {/* Header */}
        <header className="h-auto flex flex-col gap-2 items-center justify-between px-4 py-2 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 backdrop-blur">
          <div className="w-full flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => setIsSidebarOpen(true)} className="md:hidden p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
              </button>
              <div className="flex items-center gap-2">
                <span className={`w-2.5 h-2.5 rounded-full ${isActive ? 'bg-green-500 animate-pulse' : 'bg-zinc-400'}`}></span>
                <span className="font-medium text-sm">{status}</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <select value={selectedVoice} onChange={(e) => setSelectedVoice(e.target.value)} className="text-xs bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-md px-2 py-1">
           ...
           {transcriptionHistory.length === 0 && !error && (
             <div className="h-full flex flex-col items-center justify-center text-zinc-400 space-y-4">
               <button 
                 ...
                 className="flex flex-col items-center justify-center gap-4 text-zinc-400 hover:text-primary-500 dark:hover:text-primary-400 transition-colors rounded-lg p-8 disabled:opacity-50"
               >
               ...
               {micPermission === 'denied' && (
                 <p className="text-red-500 text-sm">Microphone permission is denied. Please enable it in browser settings.</p>
               )}
             </div>
           )}
             ...
             <div key={i} className="space-y-4">
               {turn.user && (
                 <div className="flex justify-end">
                   <div className="bg-primary-600 text-white px-4 py-2 rounded-2xl rounded-br-none max-w-[80%] shadow-sm">
                     {turn.user}
                   </div>
                 </div>
               )}
               {turn.model && (
                 <div className="flex justify-start">
                   <div className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-4 py-2 rounded-2xl rounded-bl-none max-w-[80%] shadow-sm">
                     {turn.model}
                   </div>
                 </div>
               )}
             ...
             <div className="opacity-50">
               {realtimeTranscript.user && (
                 <div className="flex justify-end mb-2">
                   <div className="bg-zinc-200 dark:bg-zinc-700 text-transparent bg-clip-text bg-gradient-to-r from-zinc-800 to-zinc-500 dark:from-white dark:to-zinc-400 px-4 py-2 rounded-2xl rounded-br-none max-w-[80%]">
                     {realtimeTranscript.user}
                   </div>
                 </div>
               )}
               {realtimeTranscript.model && (
                 <div className="flex justify-start">
                   <div className="bg-zinc-200 dark:bg-zinc-700 text-transparent bg-clip-text bg-gradient-to-r from-zinc-800 to-zinc-500 dark:from-white dark:to-zinc-400 px-4 py-2 rounded-2xl rounded-bl-none max-w-[80%]">
                     {realtimeTranscript.model}...
                   </div>
                 </div>
               )}
        ...
        <div className="p-4 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800">
          <div className="max-w-4xl mx-auto flex items-center justify-center gap-4">
            <div className="flex-1 flex items-center gap-2">
                ...
                <>
                    <input 
                        ...
                        className="flex-1 bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-full px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500"
                    />
                    <button 
                        onClick={sendText} 
                        className="p-3 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-full"
                    >
                    <svg className="w-5 h-5 text-zinc-600 dark:text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 12h14M12 5l7 7-7 7" /></svg>
                    </button>
                </>
                )}
            </div>
            <button 
                ...
                disabled={isConnecting}
                className={`flex items-center justify-center gap-2 px-6 py-3 rounded-full text-white font-medium shadow-lg transition-all hover:-translate-y-0.5 disabled:opacity-70 disabled:hover:translate-y-0 ${isActive ? 'bg-red-600 hover:bg-red-500' : 'bg-primary-600 hover:bg-primary-500'}`}
